<?php
namespace Qh\Api\Lib;

// RESTful Route
// implementation by Compositing on RewriteRouter
class RESTfulRoute implements \TQ\RouteInterface
{
    private $route;
    ...
    public function __construct($path, $options, $strict) {
        ...
        if ( !$strict ) {
            $this->route = new \TQ\Route\Rewrite($path, $options);
        }
        ...
        // eg. '/user/<name>/?', match '/user/haha' and match '/user/haha/'
            ...
            $this->route = new \TQ\Route\Regex("#^$path$#", $options, $keys);
        }
    }

    public function route(/*\TQ\Request_Abstract*/ $request) {
        // HTTP method adapt
        if ( $this->method !== '*' ) {
